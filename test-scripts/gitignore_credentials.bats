...
# Regression tests for the new *semaphore_credentials.txt pattern, added
# alongside the existing *gitea_credentials.txt entry to protect the
# SemaphoreUI Ansible playbook's generated DB/admin passwords and access key.

@test ".gitignore contains the new *semaphore_credentials.txt pattern alongside the existing gitea pattern" {
  grep -qF -- '*gitea_credentials.txt' "${GITIGNORE}"
  grep -qF -- '*semaphore_credentials.txt' "${GITIGNORE}"
}

@test ".gitignore's semaphore credentials pattern appears immediately after the gitea credentials pattern" {
  gitea_line="$(grep -n -F -- '*gitea_credentials.txt' "${GITIGNORE}" | head -1 | cut -d: -f1)"
  semaphore_line="$(grep -n -F -- '*semaphore_credentials.txt' "${GITIGNORE}" | head -1 | cut -d: -f1)"
  [ -n "${gitea_line}" ]
  [ -n "${semaphore_line}" ]
  [ "${semaphore_line}" -eq "$((gitea_line + 1))" ]
}

@test "git actually ignores files matching *semaphore_credentials.txt" {
  touch "${TEST_TMPDIR}/elk-wolfi_semaphore_credentials.txt"
  run git -C "${TEST_TMPDIR}" check-ignore -q "elk-wolfi_semaphore_credentials.txt"
  [ "${status}" -eq 0 ]
}

@test "git ignores nested semaphore credential files matching the pattern regardless of directory depth" {
  mkdir -p "${TEST_TMPDIR}/ansible/elk-wolfi"
  touch "${TEST_TMPDIR}/ansible/elk-wolfi/semaphore_credentials.txt"
  run git -C "${TEST_TMPDIR}" check-ignore -q "ansible/elk-wolfi/semaphore_credentials.txt"
  [ "${status}" -eq 0 ]
}

@test "git status does not report semaphore_credentials.txt as untracked" {
  touch "${TEST_TMPDIR}/semaphore_credentials.txt"
  porcelain="$(cd "${TEST_TMPDIR}" && git status --porcelain)"
  [[ "${porcelain}" != *"semaphore_credentials.txt"* ]]
}

@test "git does NOT ignore unrelated files that merely contain the word 'semaphore'" {
  # Regression guard against an overly broad pattern: files that don't end
  # in the exact 'semaphore_credentials.txt' suffix must still be tracked.
  touch "${TEST_TMPDIR}/semaphore.log" "${TEST_TMPDIR}/semaphore-stack.yaml"

  run git -C "${TEST_TMPDIR}" check-ignore -q "semaphore.log"
  [ "${status}" -ne 0 ]

  run git -C "${TEST_TMPDIR}" check-ignore -q "semaphore-stack.yaml"
  [ "${status}" -ne 0 ]
}