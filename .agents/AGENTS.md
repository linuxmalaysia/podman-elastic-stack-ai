...
4. **Git Sovereignty & Atomic Commits:** Every logical action must be committed to Git. Avoid silent execution. The AI is strictly forbidden from executing monolithic blanket commits (e.g., `git commit -am` or dumping all unrelated files into one commit). Stage and commit files granularly grouped by logical task boundaries.
