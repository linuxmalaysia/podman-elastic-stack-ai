...
- **Active Configuration Path**: Written directly to the active Windows user profile directory at `%UserProfile%\.wslconfig` (resolved dynamically inside WSL using PowerShell/cmd.exe path querying and `wslpath` translation) when explicit opt-in variable `wsl_install_active_config` is enabled.
...
- **Disk and Memory Reclamation**: Enables experimental settings such as `autoMemoryReclaim=gradual` (for gradual cached VM memory reclamation) and `sparseVhd=true` (for sparse allocation on newly created VHDs).
- **NAT Networking & DNS Tunneling**: Standardizes on `networkingMode=nat` by default to preserve Podman container port bindings while enabling `dnsTunneling=true` and `autoProxy=true` for corporate VPN compatibility.
