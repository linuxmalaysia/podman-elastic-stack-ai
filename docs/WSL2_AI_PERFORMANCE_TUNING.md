...
| **Systemd Init** | Active by default in `/etc/wsl.conf` | Active by default in `/etc/wsl.conf` |
...
### Networking Modes: Mirrored vs. NAT

WSL2 supports two primary networking modes:

1. **Mirrored Mode (`networkingMode=mirrored`)**:
   ...
   - ⚠️ **Container Warning:** Mirrored networking mode can interfere with container port bindings in Podman and Docker Desktop.

2. **NAT Mode (`networkingMode=nat`)**:
   - Default hypervisor virtual bridge.
   - Recommended when deploying heavy containerized infrastructure (such as our Podman 3-node Elasticsearch & Kibana stack).
...
### Protecting API Keys
Never commit API keys (`ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `OPENAI_API_KEY`) into configuration JSON files or git repositories.

Export API keys in your user shell initialization profile (`~/.bashrc` or `~/.zshrc`):
...
# Disable telemetry if encountering handshake timeouts
export DISABLE_TELEMETRY=1
```

Set permissions on credentials and secret files to `0600`:
```bash
chmod 0600 ~/.bashrc
```
...
WSL2 virtual hard disk files (`.vhdx`) expand dynamically as files are created but do not automatically shrink when files are deleted unless `sparseVhd=true` is active.
...
2. Shut down WSL from PowerShell:
   ...
   wsl.exe --shutdown
   ```
3. Reclaim disk space via PowerShell (Elevated Administrator):
   ```powershell
   Optimize-VHD -Path "$env:LOCALAPPDATA\Packages\CanonicalGroupLimited.Ubuntu26.04LTS_...\LocalState\ext4.vhdx" -Mode Full
   ```
   Or use `wsl --manage` (WSL 2.5+):
   ```powershell
...
```bash
# Check current system status and calculated metrics
uv run scripts/wsl2_ai_tuning.py --check

# Apply full tuning and security configurations
sudo uv run scripts/wsl2_ai_tuning.py --apply --mode=nat
```
